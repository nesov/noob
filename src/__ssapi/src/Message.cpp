...
inline Message::Message(const char* data) : 
...
inline Message::Message(std::string& data) : m_messageType(MessageType::echo), m_messageData(data) {}


inline MessageType Message::getType() const {
    return m_messageType;
}
inline std::string Message::getData() const {
    return m_messageData;
}

inline void Message::setData(const char* data) {
    m_messageData = std::string(data);
}

inline void Message::setType(MessageType type) {
    m_messageType = type;
}

inline size_t Message::size() {
    return sizeof(m_messageType) + m_messageData.size();
}

inline void Message::serialize(std::vector<char>& buffer) const {
    buffer.push_back(static_cast<char>(m_messageType));
    buffer.insert(buffer.end(), m_messageData.begin(), m_messageData.end());
}

inline Message Message::deserialize(const std::vector<char>& buffer) {
    Message message;
